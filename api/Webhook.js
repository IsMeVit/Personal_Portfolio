...
export default async function handler(req, res) {
  ...
  if (typeof telegram !== "string" || !telegramRegex.test(telegram)) {
    return res
      .status(400)
      .json({ message: "Invalid Telegram username format." });
  }
  ...
  try {
    ...
    if (!telegramRes.ok) {
      const errorData = await telegramRes
        .json()
        .catch(() => ({ description: "No error description" }));
      console.error(
        `Telegram API error: ${telegramRes.status} - ${errorData.description}`
      );
    ...
    return res
      .status(500)
      .json({ message: "Server error: Failed to send message." });
  }
}
